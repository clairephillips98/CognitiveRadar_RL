#!/bin/bash
name='_airport_cond__a8_t30_r2_e1'
radars=2
sbatch <<EOT &
#!/bin/bash
#SBATCH --nodes=1
...
python -m rl_agents.DRL_code_pytorch.Rainbow_DQN.Rainbow_DQN_radar_main --cdl=1 --epsilon_init=1 --load_model=0 --env_name="$name" --speed_layer=0 --speed_scale=1 --agents=1 --radars=2
EOT
sbatch <<EOT &
#!/bin/bash
#SBATCH --nodes=1
